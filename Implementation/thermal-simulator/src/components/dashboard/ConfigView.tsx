    ...
    <div className="flex-1 flex flex-col items-center justify-center p-8">
      <div className="max-w-2xl w-full bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-2xl shadow-xl overflow-hidden">
        <div className="p-6 border-b border-gray-100 dark:border-slate-800 flex justify-between items-center bg-gray-50 dark:bg-slate-900/50">
          ...
          <button onClick={props.onToggleTheme} className="p-2 bg-gray-200 dark:bg-slate-800 rounded-lg text-gray-700 dark:text-slate-300 hover:bg-gray-300 dark:hover:bg-slate-700 transition-colors">
        ...
        <div className="p-8 space-y-8">
          ...
          <div className="space-y-2 border-t border-gray-100 dark:border-slate-800 pt-6">
            ...
            <div className="flex items-center gap-4">
              <label className={`flex items-center gap-2 text-white px-4 py-2 rounded-lg transition-colors ${props.isUploading ? 'bg-gray-400 dark:bg-slate-700 cursor-not-allowed' : 'bg-gray-800 hover:bg-gray-900 dark:bg-slate-800 dark:hover:bg-slate-700 cursor-pointer'}`}>
                <Upload className="w-4 h-4" /> <span className="text-sm font-medium">{props.isUploading ? 'Processing...' : 'Upload Job Traces (.csv)'}</span>
                <input type="file" accept=".csv" multiple className="hidden" onChange={props.onFileUpload} disabled={props.isUploading} />
              </label>
              
              <div className="flex flex-col flex-1 max-w-[280px] gap-1">
                {props.isUploading ? (
                  <>
                  ...
                  <span className="text-sm font-medium bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 px-3 py-1 rounded-full text-center">
        ...
        <div className="p-6 bg-gray-50 dark:bg-slate-900/50 border-t border-gray-100 dark:border-slate-800">
          <button onClick={props.onLaunch} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-4 rounded-xl shadow-md transition-colors">
