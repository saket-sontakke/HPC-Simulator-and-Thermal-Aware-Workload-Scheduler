...
export default function SimulatorDashboard() {
  ...
  const [coolingEfficiency, setCoolingEfficiency] = useState<number | ''>(100);
  ...
  const scalarsRefA = useRef({
    time_elapsed_sec: 0, ambient_temp: 30, cooling_efficiency_pct: 100, jobs_completed: 0, jobs_failed: 0,
  ...
  const flushToStateA = useCallback(() => {
    ...
    setUiStateA({
      time_elapsed_sec: s.time_elapsed_sec, ambient_temp: s.ambient_temp, cooling_efficiency_pct: s.cooling_efficiency_pct, nodes: nodesRefA.current,
  ...
  const flushToStateB = useCallback(() => {
    ...
    setUiStateB({
      time_elapsed_sec: s.time_elapsed_sec, ambient_temp: s.ambient_temp, cooling_efficiency_pct: s.cooling_efficiency_pct, nodes: nodesRefB.current,
      jobs_completed: s.jobs_completed, jobs_failed: s.jobs_failed,
      queued_job_ids: s.queued_job_ids, active_job_ids: s.active_job_ids, failed_job_ids: s.failed_job_ids,
  ...
  const applyDelta = useCallback((delta: WorkerDelta, target: 'A' | 'B') => {
    ...
    s.cooling_efficiency_pct = delta.cooling_efficiency_pct;
  ...
  const applyFullState = useCallback((full: UISimulationState, target: 'A' | 'B') => {
    ...
    s.current = {
      time_elapsed_sec: full.time_elapsed_sec, ambient_temp: full.ambient_temp, cooling_efficiency_pct: full.cooling_efficiency_pct,
  ...
  const handleCoolingChange = (e: React.ChangeEvent<HTMLInputElement> | { target: { value: string } }) => {
    const val = e.target.value;
    if (val === '') { setCoolingEfficiency(''); return; }
    let num = parseInt(val);
    if (isNaN(num)) return;
    if (num > 100) num = 100;
    setCoolingEfficiency(num);
  };
  const handleCoolingBlur = () => { if (coolingEfficiency === '' || coolingEfficiency < 1) setCoolingEfficiency(1); };
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (duplicates.length > 0) setDuplicateModal({ isOpen: true, newJobs: allNewJobs, count: duplicates.length });
        ...
        setRawJobs(prev => {
          const combined = [...prev, ...allNewJobs];
          return combined.sort((a, b) => parseInt(a.id.split('-')[0]) - parseInt(b.id.split('-')[0]));
        });
      }
  ...
  const proceedWithLaunch = () => {
    ...
    const safeNodes = typeof nodeCount === 'number' ? nodeCount : 16;
    const safeTemp = typeof ambientTemp === 'number' ? ambientTemp : 30;
    const safeCooling = typeof coolingEfficiency === 'number' ? coolingEfficiency : 100;
    
    resetRefs(safeNodes, 'A');
    workerRefA.current?.postMessage({ type: 'INIT', payload: { ambientTemp: safeTemp, nodeCount: safeNodes, mode: isABTest ? 'STANDARD' : mode, coolingEfficiencyPct: safeCooling } });
    workerRefA.current?.postMessage({ type: 'ADD_JOBS', payload: { jobs: rawJobs } });

    if (isABTest) {
      resetRefs(safeNodes, 'B');
      workerRefB.current?.postMessage({ type: 'INIT', payload: { ambientTemp: safeTemp, nodeCount: safeNodes, mode: 'THERMAL_AWARE', coolingEfficiencyPct: safeCooling } });
    ...
    <div className={`${theme} min-h-screen bg-gray-50 dark:bg-slate-950 text-gray-900 dark:text-slate-200 font-sans flex flex-col transition-colors duration-300`}>
      ...
      {currentView === 'CONFIG' && (
        <ConfigView
          ...
          coolingEfficiency={coolingEfficiency} onCoolingChange={handleCoolingChange} onCoolingBlur={handleCoolingBlur}
