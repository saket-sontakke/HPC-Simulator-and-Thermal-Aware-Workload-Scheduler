"""
===============================================================================
Script Name: 01_calculate_pinn_thermal_crosstalk_priors.py
Description: Calculates thermal crosstalk coefficients (k_01 and k_10).
             It discards multicollinear noise by strictly browsing for 
             "Asymmetric Events" - periods where one GPU is under heavy load 
             while the adjacent GPU remains idle for a sustained period, 
             isolating the pure thermal bleed.
===============================================================================
"""
...
def extract_asymmetric_events(df, active_p_col, idle_p_col, idle_t_col):
    ...
    and the other is idle. Calculates crosstalk coefficient (k), delta T, and power for each event.
    """
...
def main():
    ...
    if GENERATES_FILES and results_list:
        ...
        csv_path = output_dir / "thermal_crosstalk_asymmetric_coefficients.csv"
        df_res.to_csv(csv_path, index=False)
        
        # --- TERMINAL CONCLUSIONS ---
        print("\n" + "=" * 70)
        print("=== FINAL THERMAL CROSSTALK COEFFICIENT CONCLUSIONS ===")
        print("=" * 70)
        ...
        plt.title('Average Thermal Crosstalk Coefficients (Asymmetric Events)', fontsize=14)
        for bar in bars:
            yval = bar.get_height()
            plt.text(bar.get_x() + bar.get_width()/2, yval + (yval*0.02), f"{yval:.5f} °C/W", ha='center', fontsize=11)
        plt.tight_layout()
        plt.savefig(output_dir / '1_bar_chart_asymmetric_coefficients.png', dpi=150)
        plt.close()
