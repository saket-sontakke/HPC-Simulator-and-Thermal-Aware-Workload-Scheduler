"""
===============================================================================
Script Name: calculate_pinn_thermal_capacitance_priors.py
Description: Calculates thermal capacitance (C_0 and C_1) for a PINN by
             extracting and fitting exponential step-response heating curves 
             from dual GPU telemetry logs. Enforces strict thermodynamic 
             boundaries and dynamic workload windowing.
===============================================================================
"""
...
def extract_capacitance(df, power_col, temp_col):
    ...
    and extracts C. Returns a dictionary of physics metrics or None if invalid.
    """
    ...
    # Ensure the power stays reasonably high for the first 15 seconds. 
    # If it drops immediately, it's a burst load, not a sustained heating curve.
    early_power_mean = df[power_col].iloc[start_idx : min(start_idx + 15, len(df))].mean()
    if early_power_mean < (POWER_SPIKE_W * 0.75):
        return None

    # 3. Dynamic Windowing
    # Walk forward second-by-second. Stop the window if power drops significantly, 
    # or if we hit the 150-second maximum limit.
    end_idx = start_idx
    ...
    try:
        ...
        # T_amp bounds : [MIN_T_AMP, 100.0] -> Temp must rise by at least 3C, but won't rise more than 100C.
        # tau bounds   : [5.0, 400.0] -> Takes at least 5 seconds to heat up, max ~6.5 mins to steady state.
        ...
        # If the optimizer crashed into our physical boundaries, the fit is unnatural. Discard it.
        ...
        # Final Sanity Check
        # A massive server GPU cooler should realistically be between 20 and 1000 Joules/Celsius
        ...
        # If the data is too noisy or fails to fit within our physical bounds, discard it cleanly
        return None
...
def process_single_item(item_args):
    ...
    try:
        ...
        # Resample to exactly 1-second intervals to standardize 'time' for physics equations
...
def main():
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=N_CORES) as executor:
        ...
        for future in tqdm(concurrent.futures.as_completed(futures), total=total_files, desc="Processing", unit="file"):
            ...
            if success:
                ...
                fail_count += 1
                # Only print specific errors, hide standard "no curves found" to prevent console spam
    ...
    if GENERATES_FILES and results_list:
        ...
        csv_path = output_dir / "thermal_capacitance_priors.csv"
        df_res.to_csv(csv_path, index=False)
        
        # --- TERMINAL CONCLUSIONS ---
        print("\n" + "=" * 70)
        print("=== FINAL PINN CAPACITANCE CONCLUSIONS ===")
        print("=" * 70)
        ...
        plt.title('Distribution of Capacitance Across MIT Supercloud Cluster', fontsize=14)
        plt.legend()
        plt.tight_layout()
        ...
        # Generate 150 seconds of idealized time data
    ...
    if GENERATES_FILES:
        print(f"\n[!] Outputs, terminal logs, and graph saved to: {output_dir}")
    else:
