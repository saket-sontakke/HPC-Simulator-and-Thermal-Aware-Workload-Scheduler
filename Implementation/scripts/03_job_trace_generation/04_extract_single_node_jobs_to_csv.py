# """
# ===============================================================================
# Script Name: 04_extract_single_gpu_jobs_to_csv.py
...
"""
===============================================================================
Script Name: 04_extract_single_node_jobs_to_csv.py
Description: Scans thousands of nvidia-smi parquet logs, groups them by Job ID
             extracted from the filename, and filters for jobs that ran on a 
             SINGLE NODE (Job ID appears exactly once). Converts to .csv.
===============================================================================
"""
...
N_CORES = 8             # Default cores for parallel processing
GENERATES_FILES = True  # SET TO FALSE if this script ONLY prints to terminal
CREATE_DATA_SYMLINK = True # Routes heavy data to a dedicated data folder
...
def process_single_item(item_args):
    ...
    Since filtering happens BEFORE multiprocessing now, this function 
    simply handles the Parquet-to-CSV conversion.
    """
    file_path, output_dir = item_args
    try:
        if GENERATES_FILES:
...
def main():
    ...
    if GENERATES_FILES:
        ...
        if CREATE_DATA_SYMLINK:
            # Updated folder name based on your request
            actual_output_dir = data_dir.parent / "labelled_jobs_single_node_csv"
            actual_output_dir.mkdir(parents=True, exist_ok=True)
            
            # Create shortcut
            shortcut_path = log_dir / "labelled_jobs_single_node_csv_shortcut"
    ...
    # --- NEW LOGIC: Pre-filtering for Single Node Jobs ---
    print(f"Analyzing {total_files} files to identify single-node jobs...")
    
    # 1. Count occurrences of every Job ID
    job_id_counts = Counter()
    for fp in target_files:
        # Assuming format: job_id-rack_id-node_id.parquet
        job_id = fp.stem.split('-')[0]
        job_id_counts[job_id] += 1

    # 2. Filter files to keep only those where Job ID appears exactly once
    single_node_files = []
    for fp in target_files:
        job_id = fp.stem.split('-')[0]
        if job_id_counts[job_id] == 1:
            single_node_files.append(fp)

    multi_node_files = total_files - len(single_node_files)
    
    print(f"Found {len(single_node_files)} single-node jobs.")
    print(f"Filtered out {multi_node_files} multi-node job segments.\n")

    # --- Parallel Processing ---
    if len(single_node_files) > 0:
        print(f"Starting parallel CSV conversion on {len(single_node_files)} files using {N_CORES} cores...\n")
        
        # Only pass the validated single-node files to the workers
        tasks = [(fp, actual_output_dir) for fp in single_node_files]
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=N_CORES) as executor:
            ...
            for future in tqdm(concurrent.futures.as_completed(futures), total=len(single_node_files), desc="Converting", unit="file"):
                success, filename, status_msg = future.result()
        ...
        overall_converted = 0
        overall_fail = 0
        print("[!] No single-node jobs found to process.")
    ...
    print(f"Single Node Jobs    : {overall_converted} (Converted to CSV)")
    print(f"Multi Node Files    : {multi_node_files} (Skipped completely)")
    if overall_fail > 0:
        print(f"Failed to read/save : {overall_fail}")
    print(f"Total Time          : {formatted_time}")
